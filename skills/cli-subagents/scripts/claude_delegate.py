...
def classify_failure(returncode: int, stdout: str, stderr: str) -> str | None:
    ...
    if (
        ...
        or "organization has been disabled" in text
    ):
    ...
    if "overloaded" in text or "529" in text or "temporarily limiting" in text:
        return "rate_limit_or_quota"
    if "credit balance" in text or "usage credits required" in text or "out of usage credits" in text:
        return "rate_limit_or_quota"
...
def build_claude_run_argv(
    ...
    *,
    ...
    effort = args.effort or DEFAULT_CLAUDE_EFFORT
...
def do_run(args: argparse.Namespace) -> int:
    ...
    failure_kind = None if ok else (
        "empty_result" if result.returncode == 0 else classify_failure(result.returncode, result.stdout, result.stderr)
    )
    payload.update(
        {
            "ok": ok,
