...
def build_grok_argv(
    ...
    *,
    ...
    argv = [
        ...
        "--sandbox",
        "read-only" if probe else MODE_SANDBOX[mode],
        "--max-turns",
    ...
    argv.extend(["--effort", args.effort or DEFAULT_GROK_EFFORT])
...
def do_probe(args: argparse.Namespace) -> int:
    ...
    args.max_turns = 1
    args.model = getattr(args, "model", None) or DEFAULT_GROK_MODEL
    args.effort = getattr(args, "effort", None) or DEFAULT_GROK_EFFORT
    args.rule = []
    args.allow = []
    args.deny = []
...
def do_audit_prompt(args: argparse.Namespace) -> int:
    ...
    json_print({"ok": True, "status": "safe", "issues": []})
    return 0
...
def do_run(args: argparse.Namespace) -> int:
    ...
    if result.returncode != 0:
        ...
        output_failure = semantic_failure
    else:
        output_failure = None if result.stdout.strip() else "empty_result"
    ok = result.returncode == 0 and output_failure is None
    payload.update(
        {
            "ok": ok,
            "status": "complete" if ok else "blocked",
            "failure_kind": output_failure or classify_failure(result.returncode, result.stdout, result.stderr),
            "result": {
                "returncode": result.returncode,
                "stdout": result.stdout,
                "stderr": result.stderr,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    probe.add_argument("--effort", choices=["low", "medium", "high"])
