...
class OpenCodeDelegateTests(unittest.TestCase):
    def test_run_requires_explicit_inactive_opt_in(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = run_wrapper(
                ["run", "--opencode-bin", str(fake), "--cwd", str(root), "--prompt", "Review this."]
            )
            payload = json.loads(result.stdout)

            self.assertEqual(result.returncode, 2)
            self.assertEqual(payload["status"], "inactive")
            self.assertIn("opencode_inactive_requires_explicit_opt_in", payload["issues"])
    ...
    def test_doctor_supports_non_ollama_provider_models(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            fake = make_fake_opencode(
                root,
    ...
    def test_run_accepts_context_allowed_by_workspace_policy(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            fake = make_fake_opencode(
                ...
                print(json.dumps({{"type":"text","part":{{"type":"text","text":"reviewed"}}}}))
                raise SystemExit(0)
                """,
            )
            private_prompt = (
    ...
    def test_probe_requires_exact_ready_from_json(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                payload["command"]["argv"][payload["command"]["argv"].index("--agent") + 1],
                "plan",
            )
